...
void main()
{
    gl_Position = vec4(vin_pos.x + v_pos.x, vin_pos.y + v_pos.y, vin_pos.z + v_pos.z, 1.0);
    
    // 정점 ID에 따라 각각 R/G/B 한 채널만 전달
    if (vin_color.r > 0.0)
        vout_color = vec4(vin_color.r + u_color.r,0,0, 1.0); // 첫 번째 정점: R
    else if (vin_color.g > 0.0)
        vout_color = vec4(0.0, vin_color.g + u_color.g, 0.0, 1.0); // 두 번째 정점: G
    else
        vout_color = vec4(0.0, 0.0, vin_color.b + u_color.b, 1.0); // 세 번째 정점: B
}
'''
...
def main():
    ...
    vertices = glm.array(glm.float32,
        ...
        0.0, 0.5, 0.0, # left vertex x, y, z coordinates
        -0.5, -0.5, 0.0, # right vertex x, y, z coordinates
        0.5, 0.0, 0.0, # right vertex x, y, z coordinates
         0.0, 0.5, 0.0,  # top vertex x, y, z coordinates
        0.0, 0.0, 0.5,  # top vertex x, y, z coordinates
    )
    ...
    while not glfwWindowShouldClose(window):
        ...
        glUniform3f(loc_u_pos, pos, 0, pos)
